...
// This does not work on android...
        ...
        public /*static*/ float GetATGRandomNext() {
            ATGSeed = (uint)(((ulong)ATGSeed * (ulong)279470273) % (ulong)4294967291);
            return (float)ATGSeed * OneOverInt32MaxVal;
        }


        // https://stackoverflow.com/questions/20935994/custom-random-number-generator
            ...
            static uint xs;
            static uint xy;
            static float getRandomFloat(uint x, uint y, uint seed) {
...
//  The real update function which culls and queues grass cells and draws the finished ones
        ...
        //  HiZ Buffer - currently not supported in URP
            if (UseCompute && !usingURP) {
                if(HiZBuffer == null) {
        ...
        //  Compute: As the HiZBuffer may not be available.
            if (usingURP == false) {
                if (UseCompute && HiZBuffer.HiZavailable == false)
                    return;
            }
                        ...
                        for (int j = 0; j < NumberOfLayersInCell; j++) {
                            ...
                            if (!UseCompute) {
                                if (CurrentCellContent.state == 3)
                                    CurrentCellContent.DrawCellContent_Delegated( CameraInWichGrassWillBeDrawn, CameraLayer, TerrainShift);
                            }
                            else {
                                if (CurrentCellContent.state == 3)
                                {
                        ...
                        if (!UseCompute) {
                            ...
                            for (int j = 0; j < NumberOfLayersInCell; j++) {
                                if(CellContent[CurrentCell.CellContentIndexes[j]].state == 3)
                                    CellContent[ CurrentCell.CellContentIndexes[j] ].DrawCellContent_Delegated( CameraInWichGrassWillBeDrawn, CameraLayer, TerrainShift);
                            }
                            UnityEngine.Profiling.Profiler.EndSample();
                        }
                        break;
                    ...
                    //  Tweak layer to point at softly merged Layer
                        ...
                        for(int x = 0; x < NumberOfBucketsPerCell; x++) {
                            ...
                            //  Creating a random for each bucket fixes the problem of jumping grass when some of it is removed...
                                int layerRandomBase = cellIndex + layer + layers * 53; // 55 / 9949;
                            //  ( 0.0001f * (x+z)) ---> results in curved lines
                                #if UNITY_ANDROID
                                    ATGSeed = (uint)(layerRandomBase - (x * NumberOfBucketsPerCell + z));
                                #else
                                    ATGSeed = (uint)((layerRandomBase - (x * NumberOfBucketsPerCell + z)) * ( 0.0001f * (x + z) * NumberOfBucketsPerCell )  * (float)uint.MaxValue);
                                #endif
...
*/
                            ...
                            //  Now that we have the density for the Bucket we spawn x instances based on density within the given Bucket.
                                ...
                                //  Random Offsets
                                    #if UNITY_ANDROID
                                        float rand = getRandomFloat((uint)i, 0, ATGSeed);
                                    #else
                                        float rand = GetATGRandomNext(); //(float)random.NextDouble();
                                    #endif

                                    float XOffset = rand * BucketSize;
                                    #if UNITY_ANDROID
                                        rand = getRandomFloat((uint)i, 1, ATGSeed);
                                    #else
                                        rand = GetATGRandomNext(); //(float)random.NextDouble();
                                    #endif
...
*/
                                ...
                                //  Now add random rotation around y axis
                                    #if UNITY_ANDROID
                                        float HalfAngle = getRandomFloat((uint)i, 2, ATGSeed) * 180.0f;
                                    #else
                                        float HalfAngle = GetATGRandomNext() * 180.0f; //(float)random.Next(0, 180);
                                    #endif
                                ...
                                //  Rotation around x axis (for e.g. small stones)
                                    if(Rotation == 1) {
                                        #if UNITY_ANDROID
                                            HalfAngle = getRandomFloat((uint)i, 3, ATGSeed) * 180.0f;
                                        #else
                                            HalfAngle = GetATGRandomNext() * 180.0f; //(float)random.Next(0, 180);
                                        #endif
